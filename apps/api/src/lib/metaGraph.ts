/**
 * Meta Graph API helper utilities.
 * Provides shared fetch functions (GET/POST) with timeout and error mapping.
 */
...
function mapMetaError(error: { message?: string; type?: string; code?: number; error_subcode?: number } | undefined, defaultMessage: string): MetaGraphError {
  ...
  if (error) {
    if (error.code === 190 || error.type === "OAuthException") {
      ...
      hint = "Missing required permissions. Reconnect Meta with proper scopes.";
    } else if (error.code === 4) {
      hint = "Rate limit exceeded. Please try again later.";
    } else if (error.code === 10) {
      hint = "Permission denied. Check your Meta app permissions.";
    } else if (error.code === 100) {
      hint = "Invalid parameter. Check your request body and Meta API documentation.";
    }
  }
 ...
 * @param path - Graph API path (e.g., "/me" or "/me/adaccounts")
 * @param token - Access token
 * @param options - Optional fetch options (timeout, fields, etc.)
